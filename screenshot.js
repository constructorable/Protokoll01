...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('screenshot').addEventListener('click', async function () {
        ...
        if (!canProceed) {
            return; // PDF-Erstellung abbrechen
        }
            ...
            for (let i = 0; i < availableSections.length; i++) {
                ...
                if (section.type === 'combined') {
                    ...
                    try {
                        ...
                        // CHECKBOX-STYLES VOR html2canvas SETZEN
...
function replaceCheckboxesForPDF(container) {
    ...
    checkboxes.forEach((checkbox, index) => {
        // Original-Checkbox speichern
        const originalCheckbox = checkbox.cloneNode(true);
        
        // Container-Div für die Zentrierung erstellen
        ...
        // Häkchen hinzufügen wenn aktiviert
        if (checkbox.checked) {
        ...
        // Checkbox in Wrapper einbetten
        wrapper.appendChild(replacement);
        
        // Original-Checkbox durch Wrapper ersetzen
        checkbox.parentNode.replaceChild(wrapper, checkbox);
        
        // Für spätere Wiederherstellung speichern
        replacements.push({
            replacement: wrapper,
...
// Funktion um Original-Checkboxen wiederherzustellen
